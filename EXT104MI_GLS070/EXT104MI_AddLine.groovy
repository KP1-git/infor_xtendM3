...
public class AddLine extends ExtendM3Transaction {
	...
	public void main() {
		...
		if(!fgdiseRecord.read(fgdiseContainer)){
			fgdiseContainer.setString("BVTX40", tx40)
			fgdiseContainer.setString("BVTX15", tx15)
			fgdiseContainer.set("BVBDLV", bdlv)
			fgdiseContainer.setString("BVCFLV", cflv)
			fgdiseContainer.setString("BVCTLV", ctlv)
			fgdiseContainer.setString("BVBTAB", btab)
			fgdiseContainer.setString("BVTTAB", ttab)
			fgdiseContainer.set("BVCABF", cabf)
			fgdiseContainer.set("BVBUN1", bun1)
			fgdiseContainer.setString("BVBVE1", bve1)
			fgdiseContainer.set("BVCASF", casf)
			fgdiseContainer.set("BVBUN2", bun2)
			fgdiseContainer.setString("BVBVE2", bve2)
			fgdiseContainer.setString("BVRDUV", rduv)

			fgdiseContainer.set("BVRGDT", (Integer) utility.call("DateUtil", "currentDateY8AsInt"))
			fgdiseContainer.set("BVLMDT", (Integer) utility.call("DateUtil", "currentDateY8AsInt"))
			fgdiseContainer.set("BVCHID", program.getUser())
			fgdiseContainer.set("BVRGTM", (Integer) utility.call("DateUtil", "currentTimeAsInt"))
			fgdiseContainer.set("BVCHNO", 1)
			fgdiseRecord.insert(fgdiseContainer)
		}else
		{
